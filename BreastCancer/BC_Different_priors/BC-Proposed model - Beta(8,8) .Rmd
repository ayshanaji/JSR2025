...
```{r}
# Save the results
save(BETA, ZLV,THETA,GLOBAL_THETA,theta_j, file = "BC-Proposed model_Beta(8,8).RData")

# To load the results back in future sessions
load("BC-Proposed model_Beta(8,8).RData")
...
# Save as PDF table
pdf("ESS_Summary_PostThinning_flexible_beta_Beta(8,8).pdf", width = 8, height = 5)
...
# Run diagnostics
plot_trace_density_to_pdf(BETA_thinned, variable_names, "TraceDensity_Beta_PostThinning_flexible_beta_Beta(8,8).pdf")
plot_trace_density_to_pdf(BETA_ZLV_thinned, variable_names, "TraceDensity_BetaZ_PostThinning_flexible_beta_Beta(8,8).pdf")
plot_acf_to_pdf(BETA_thinned, variable_names, "ACF_Beta_PostThinning_flexible_beta_Beta(8,8).pdf")
plot_acf_to_pdf(BETA_ZLV_thinned, variable_names, "ACF_BetaZ_PostThinning_flexible_beta_Beta(8,8).pdf")
...
pdf("TraceDensity_GlobalTheta_PostThinning_flexible_beta_Beta(8,8).pdf", width = 8, height = 4)
par(mfrow = c(1, 2), mar = c(4, 4, 3, 1))
...
# ACF plot
pdf("ACF_GlobalTheta_PostThinning_flexible_beta_Beta(8,8).pdf", width = 6, height = 4)
...
```{r}
png("trace_global_theta_CAD_Beta(8,8).png", width = 800, height = 600)
...
```
```{r}
png("density_global_theta_CAD_Beta(8,8).png", width = 800, height = 600)
...
acf(GLOBAL_THETA)
png("acf_global_theta_CAD_Beta(8,8).png", width = 800, height = 600)
...
pdf("CI_PSP_Paper_flexible_CAD_Beta(8,8).pdf", width = 12, height = 7)
...
pdf("CI_PSP_Presentation_flexible_CAD_Beta(8,8).pdf", width = 12, height = 7)
...
write.csv(csv_out, "OR_PSP_AllPredictors_flexible_beta_Beta(8,8).csv", row.names = FALSE)
...
writeLines(latex_code_paper, con = "OR_PSP_Table_NoIntercept_PAPER_flexible_beta_Beta(8,8).tex")
...
writeLines(latex_code_present, con = "OR_PSP_Table_NoIntercept_PRESENT_flexible_beta_bc_(8,8).tex")
...
write.csv(csv_out, "OR_PSP_AllPredictors_flexible_CAD_Beta(8,8).csv", row.names = FALSE)
...
pdf("performance_metrics_comparison_Beta(8,8).pdf", width = 8, height = 11)

...
```{r}
# ─────────────────────────────────────────────
# 📌 SECTION: Load Data & Define Logistic Likelihood
# Description:
...
```{r}
# ─────────────────────────────────────────────
# 📌 SECTION: Frequentist Initialization & Prior Setup
# Description:
...
pmn.beta <- BETA_init     # Prior mean for β
...
accept_zlv  <- rep(0, k)  # Per-variable zlv acceptance tracker
...
```{r}
# ─────────────────────────────────────────────
# 📌 SECTION: Tune Proposal Variance (Scaling Factor c=W_beta)
# Description:
...
start_time <- Sys.time()
var.prop <- c * vcov_matrix
for (i in 1:20) {  # Adjust the proposal variance over 10 iterations of tuning
...
```{r}
# ─────────────────────────────────────────────
# 📌 SECTION: Full MCMC Sampling (β and Z)
# Description:
# - Metropolis-Hastings step for β
...
for (s in 1:S) {
  ...
  # ── Step 2: Update zlv (variable inclusion) ──
  for (j in 1:k) {
    zlv.p <- zlv
...
```{r}
# ─────────────────────────────────────────────
# 📌 SECTION: Save & Reload Posterior Samples
# Description:
# - Save BETA and ZLV matrices as .RData
# - Reload from file when needed
# ─────────────────────────────────────────────
# Save the results
save(BETA, ZLV, file = "Standard_model_results.RData")

# To load the results back in future sessions
#load("Standard_model_results.RData")
```
...
# Run diagnostics
plot_trace_density_to_pdf(BETA_thinned, variable_names, "TraceDensity_Beta_PostThinning_Fixed.pdf")
plot_trace_density_to_pdf(BETA_ZLV_thinned, variable_names, "TraceDensity_BetaZ_PostThinning_Fixed.pdf")
plot_acf_to_pdf(BETA_thinned, variable_names, "ACF_Beta_PostThinning_Fixed.pdf")
plot_acf_to_pdf(BETA_ZLV_thinned, variable_names, "ACF_BetaZ_PostThinning_Fixed.pdf")
...
```{r}
# ─────────────────────────────────────────────
# 📌 SECTION: Posterior Model Probabilities (Top-5 Models)
# Description:
...
# ── Plot posterior probabilities of Top-5 Models ──
...
# Two exports:
#   1) Paper (black/gray)      → "CI_PSP_Paper_Fixed.pdf"
#   2) Presentation (red/blue) → "CI_PSP_Presentation_Fixed.pdf"
# Notes:
...
library(ggplot2)
library(stringr)   # <-- CHANGE: for label wrapping
...
# Order from HIGH to LOW PSP
combined <- combined[order(-combined$PSP), ]

# ---- CHANGE: wrap labels and reverse so highest PSP is on top after coord_flip()
wrap_width <- 22                                 # tweak if names are very long
combined$Label_wrapped <- str_wrap(combined$Variable, width = wrap_width)
combined$Label_wrapped <- factor(combined$Label_wrapped, levels = rev(combined$Label_wrapped))

# ---- CHANGE: automatically choose a taller PDF for many labels
n_labels <- nrow(combined)
height_in <- max(7, 0.22 * n_labels)            # ~0.22 inches per label works well

# ── STYLE 1: PAPER (black/gray) ──────────────────────────────
p_ci_paper <- ggplot(combined, aes(x = Label_wrapped, y = Median)) +
  geom_point(size = 2.8, aes(color = Significance)) +
  geom_errorbar(aes(ymin = Lower, ymax = Upper, color = Significance), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_color_manual(values = c("Significant" = "black", "Not significant" = "gray")) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 7)        # <-- CHANGE: smaller y-axis text
  )

p_psp_paper <- ggplot(combined, aes(x = Label_wrapped, y = PSP, fill = Significance)) +
  geom_bar(stat = "identity", width = 0.8) +
  scale_fill_manual(values = c("Significant" = "black", "Not significant" = "gray")) +
  coord_flip() +
  labs(title = "Posterior selection probability", x = NULL, y = NULL) +
  ...
  theme(
    legend.position = "none",
    axis.text.y = element_blank()               # keep blank so labels aren’t repeated
  )

pdf("CI_PSP_Paper_fixed_BC.pdf", width = 12, height = height_in)   # <-- CHANGE
grid.arrange(p_ci_paper, p_psp_paper, ncol = 2, widths = c(0.65, 0.35))
dev.off()

# ── STYLE 2: PRESENTATION (red dots + blue intervals/bars) ──
p_ci_pres <- ggplot(combined, aes(x = Label_wrapped, y = Median)) +
  geom_point(size = 3.2, aes(color = Significance)) +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.25, color = "steelblue") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_color_manual(values = c("Significant" = "red", "Not significant" = "gray")) +
  coord_flip() +
  labs(title = "Credible intervals", x = NULL, y = NULL) +
  ...
  theme(
    legend.position = "none",
    axis.text.y = element_text(size = 7)        # <-- CHANGE
  )

p_psp_pres <- ggplot(combined, aes(x = Label_wrapped, y = PSP)) +
  geom_bar(stat = "identity", width = 0.8, fill = "steelblue") +
  coord_flip() +
  labs(title = "Posterior selection probability", x = NULL, y = NULL) +
  theme_minimal(base_size = 13) +
...
pdf("CI_PSP_Presentation_fixed_BC.pdf", width = 12, height = height_in)  # <-- CHANGE
grid.arrange(p_ci_pres, p_psp_pres, ncol = 2, widths = c(0.65, 0.35))
dev.off()
...
```{r}
# ─────────────────────────────────────────────────────────────
# Log-odds + PSP Tables (CSV incl. intercept; LaTeX w/o intercept)
# Exports:
#   • CSV (includes intercept):            LOGODDS_PSP_AllPredictors_fixed.csv
#   • LaTeX (paper, no Width cols):        LOGODDS_PSP_Table_NoIntercept_PAPER_fixed.tex
#   • LaTeX (presentation, with Width & %):LOGODDS_PSP_Table_NoIntercept_PRESENT_fixed.tex
# Notes:
#   • Quantiles are for β·Z (post-thinning) on the LOG-ODDS scale (NO exp()).
#   • PSP = mean(Z).
#   • LaTeX tables EXCLUDE intercept; rows ordered by PSP (high → low).
# ─────────────────────────────────────────────────────────────
...
beta_df <- data.frame(
  ...
  Lower = beta_q[, 1],   # log-odds (no exp)
  Median = beta_q[, 2],  # log-odds (no exp)
  Upper = beta_q[, 3],   # log-odds (no exp)
  row.names = NULL
)
...
combined <- merge(beta_df, psp_df, by = "Variable", all.x = TRUE)
combined$Label    <- combined$Variable
combined$Width    <- combined$Upper - combined$Lower                 # absolute width (log-odds)
combined$WidthPct <- ifelse(abs(combined$Median) < .Machine$double.eps, 
                            NA_real_,
                            100 * (combined$Upper - combined$Lower) / abs(combined$Median))  # % relative to |median|

# 5) Order by PSP (high → low)
combined <- combined[order(-combined$PSP), ]

# 6) CSV (INCLUDES INTERCEPT) for full record
csv_out <- combined[, c("Variable", "Label", "Median", "Lower", "Upper", "Width", "WidthPct", "PSP")]
write.csv(csv_out, "LOGODDS_PSP_AllPredictors_fixed.csv", row.names = FALSE)
...
latex_paper <- latex_fmt[, c("Label", "Median", "Lower", "Upper", "PSP")]
colnames(latex_paper) <- c("Predictor", "Log-odds (50\\%)", "2.5\\% CI", "97.5\\% CI", "PSP")
...
latex_code_paper <- knitr::kable(
  ...
  booktabs = TRUE,
  align    = c("l","c","c","c","c"),
  caption  = "Log-odds with 95\\% credible intervals and Posterior Selection Probability (PSP). Intercept excluded."
)

writeLines(latex_code_paper, con = "LOGODDS_PSP_Table_NoIntercept_PAPER_fixed.tex")
cat(latex_code_paper)

# ── PRESENTATION VERSION (adds Width + % on LOG-ODDS scale) ─
latex_present <- latex_fmt[, c("Label", "Median", "Lower", "Upper", "Width", "WidthPct", "PSP")]
latex_present$WidthPct <- ifelse(is.na(latex_present$WidthPct), "--", paste0(latex_present$WidthPct, "\\%"))

colnames(latex_present) <- c("Predictor", "Log-odds (50\\%)", "2.5\\% CI", "97.5\\% CI",
                             "Width", "Width (\\% of |median|)", "PSP")
...
latex_code_present <- knitr::kable(
  ...
  caption  = "Log-odds with 95\\% credible intervals, width (on log-odds), relative width, and PSP. Intercept excluded."
)

writeLines(latex_code_present, con = "LOGODDS_PSP_Table_NoIntercept_PRESENT_fixed.tex")
cat(latex_code_present)
...
# Exports:
#   • CSV (includes intercept):            OR_PSP_AllPredictors_Fixed.csv
#   • LaTeX (paper, no Width cols):        OR_PSP_Table_NoIntercept_PAPER_Fixed.tex
#   • LaTeX (presentation, with Width & %):OR_PSP_Table_NoIntercept_PRESENT_Fixed.tex
# Notes:

...
```{r}
# Save the results
save(BETA, ZLV,THETA,GLOBAL_THETA,theta_j, file = "BC-Proposed model_Beta(1,1).RData")

# To load the results back in future sessions
load("BC-Proposed model_Beta(1,1).RData")
...
```{r}
png("trace_global_theta_CAD_Beta(1,1).png", width = 800, height = 600)
...
```
```{r}
png("density_global_theta_CAD_Beta(1,1).png", width = 800, height = 600)
...
acf(GLOBAL_THETA)
png("acf_global_theta_CAD_Beta(1,1).png", width = 800, height = 600)
...
pdf("CI_PSP_Paper_flexible_BC_Beta(1,1).pdf", width = 12, height = 7)
...
pdf("CI_PSP_Presentation_flexible_BC_Beta(1,1).pdf", width = 12, height = 7)
...
pdf("CI_PSP_Paper_flexible_BC_Beta(1,1).pdf", width = 12, height = height_in)   # <-- CHANGE
grid.arrange(p_ci_paper, p_psp_paper, ncol = 2, widths = c(0.65, 0.35))
dev.off()
...
pdf("CI_PSP_Presentation_flexible_BC_Beta(1,1).pdf", width = 12, height = height_in)  # <-- CHANGE
grid.arrange(p_ci_pres, p_psp_pres, ncol = 2, widths = c(0.65, 0.35))
dev.off()
```
...
write.csv(csv_out, "LOGODDS_PSP_AllPredictors_flexible_bc.csv", row.names = FALSE)
...
writeLines(latex_code_paper, con = "LOGODDS_PSP_Table_NoIntercept_PAPER_flexible_beta(1,1).tex")
...
writeLines(latex_code_present, con = "LOGODDS_PSP_Table_NoIntercept_PRESENT_flexible_beta(1,1).tex")
...
write.csv(csv_out, "OR_PSP_AllPredictors_flexible_CAD_Beta(1,1).csv", row.names = FALSE)
...
pdf("performance_metrics_comparison_Beta(1,1).pdf", width = 8, height = 11)

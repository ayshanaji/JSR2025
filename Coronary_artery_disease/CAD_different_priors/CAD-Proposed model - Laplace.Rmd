...
# ------------------ Solve b for Laplace(μ, b) to have 99% mass in (0,1)
solve_b_theta_01 <- function(mu = 0.5, p = 0.99) {
  trunc_mass <- function(b) {
    F0 <- Laplace_CDF(0, mu, b)
    F1 <- Laplace_CDF(1, mu, b)
    F1 - F0
  }
  uniroot(function(b) trunc_mass(b) - p, c(1e-4, 50))$root
}

# ------------------ Choose center and coverage ------------------------
theta0  <- 0.50   # Prior center (location of Laplace)
pcover  <- 0.99   # Desired coverage in (0,1)

# ------------------ Calibrate scale b_theta ---------------------------
b_theta <- solve_b_theta_01(mu = theta0, p = pcover)

cat(sprintf("Calibrated Laplace scale b_theta = %.6f\n", b_theta))
cat(sprintf("Check prior mass in (0,1) ≈ %.4f\n",
            Laplace_CDF(1, theta0, b_theta) - Laplace_CDF(0, theta0, b_theta)))

# ------------------ Laplace log-density function -----------------------
# This is unnormalized (truncation constant cancels out in Metropolis steps)
dlaplace <- function(x, location = theta0, scale = b_theta) {
  -log(2 * scale) - abs(x - location) / scale
}
...
# flogis.log(beta, zlv, X, y) : your logistic log-lik given z-masking
...
eps      <- 1e-12
kappa    <- 20                 # Beta-proposal concentration for theta

# --- init --------------------------------------------------------------------
global_theta <- 0.5
theta_j      <- rbeta(k, global_theta, 1 - global_theta)
...
for (s in 1:S) {
  ...
  if (log(runif(1)) < lhr) { beta <- beta.p; acsb <- acsb + 1L }
  BETA[s, ] <- beta

  # ---- Step 2: MH for zlv ----------------------------------------------------
  theta_j <- clamp01(theta_j, eps)
  for (j in 1:k) {
    zlv.p    <- zlv; zlv.p[j] <- 1L - zlv[j]
    ...
    lhg <- ll_diff + lp_diff
    if (log(runif(1)) < lhg) { zlv[j] <- zlv.p[j]; acsg <- acsg + 1L }
  }
  ZLV[s, ] <- zlv

  # ---- Step 3: Gibbs for theta_j ---------------------------------------------
  for (j in 1:k) {
    theta_j[j] <- rbeta(1,
  ...
  # ---- Step 4: MH for global_theta with Laplace prior & Beta proposal -------
  global_theta <- clamp01(global_theta, eps)
  ...
  # target: ∏_j Beta(theta_j ; theta, 1-theta) * Laplace(theta ; theta0, b_theta)
  log_num <- sum(dbeta(theta_j, proposed_global_theta,
                       1 - proposed_global_theta, log = TRUE)) +
             dlaplace(proposed_global_theta)

  log_den <- sum(dbeta(theta_j, global_theta,
                       1 - global_theta, log = TRUE)) +
             dlaplace(global_theta)

  # Hastings correction for asymmetric Beta proposal q(·|θ)
...
# --- Posterior summaries -----------------------------------------------------
post_idx <- (burn_in + 1):S
posterior_mean_theta      <- mean(GLOBAL_THETA[post_idx])
posterior_sd_theta        <- sd(GLOBAL_THETA[post_idx])
credible_interval_theta   <- quantile(GLOBAL_THETA[post_idx], probs = c(0.005, 0.995))

cat("Posterior mean of global_theta:", posterior_mean_theta, "\n")
cat("Posterior SD of global_theta:",   posterior_sd_theta,   "\n")
cat("99% credible interval for global_theta:", credible_interval_theta, "\n")
...
```{r}
# Save the results
save(BETA, ZLV,THETA,GLOBAL_THETA,theta_j, file = "CAD-Proposed model_laplace.RData")

# To load the results back in future sessions
load("CAD-Proposed model_laplace.RData")
...
# Save as PDF table
pdf("ESS_Summary_PostThinning_flexible.pdf", width = 8, height = 5)
...
# Run diagnostics
plot_trace_density_to_pdf(BETA_thinned, pretty_labels, "TraceDensity_Beta_PostThinning_flexible_laplace.pdf")
plot_trace_density_to_pdf(BETA_ZLV_thinned, pretty_labels, "TraceDensity_BetaZ_PostThinning_flexible_laplace.pdf")
plot_acf_to_pdf(BETA_thinned, pretty_labels, "ACF_Beta_PostThinning_flexible_laplace.pdf")
plot_acf_to_pdf(BETA_ZLV_thinned, pretty_labels, "ACF_BetaZ_PostThinning_flexible_laplace.pdf")
...
pdf("TraceDensity_GlobalTheta_PostThinning_flexible_laplace.pdf", width = 8, height = 4)
par(mfrow = c(1, 2), mar = c(4, 4, 3, 1))
...
# ACF plot
pdf("ACF_GlobalTheta_PostThinning_flexible_laplace.pdf", width = 6, height = 4)
...
```{r}
png("trace_global_theta_CAD_laplace.png", width = 800, height = 600)
...
```
```{r}
png("density_global_theta_CAD_laplace.png", width = 800, height = 600)
...
acf(GLOBAL_THETA)
png("acf_global_theta_CAD_laplace.png", width = 800, height = 600)
...
if (!exists("pretty_name")) {
  pretty_labels_map <- c(
    "(Intercept)" = "Intercept",
    # short tags used earlier
    "MHR"         = "Max HR",
    "RBPS"        = "Resting BP",
    "Chol"        = "Cholesterol",
    "AgeGrp"      = "Age group",
    # snake_case variants (your request)
    "max_hr"      = "Max HR",
...
pdf("CI_PSP_Paper_flexible_CAD_laplace.pdf", width = 12, height = 7)
...
pdf("CI_PSP_Presentation_flexible_CAD_laplace.pdf", width = 12, height = 7)
...
write.csv(csv_out, "OR_PSP_AllPredictors_flexible_laplace.csv", row.names = FALSE)
...
writeLines(latex_code_paper, con = "OR_PSP_Table_NoIntercept_PAPER_flexible_laplace.tex")
...
writeLines(latex_code_present, con = "OR_PSP_Table_NoIntercept_PRESENT_flexible_laplace.tex")
...
write.csv(csv_out, "OR_PSP_AllPredictors_flexible_CAD_laplace.csv", row.names = FALSE)
...
pdf("performance_metrics_comparison_laplace.pdf", width = 8, height = 11)

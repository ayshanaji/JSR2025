...
# --- init --------------------------------------------------------------------
global_theta <- rbeta(1, alpha1, alpha2)                 # global sparsity
...
for (s in 1:S) {
  ...
  ZLV[s, ] <- zlv
  # Step 3: Gibbs update for each theta_j from Beta(global_theta + zlv[j], (1 - global_theta) + (1 - zlv[j]))
  eps <- 1e-8  # small safeguard to avoid exact 0 or 1
  for (j in 1:k) {
    theta_j[j] <- rbeta(1, global_theta + zlv[j], (1 - global_theta) + (1 - zlv[j]))
    # keep strictly inside (0,1)
    theta_j[j] <- min(max(theta_j[j], eps), 1 - eps)
  }
  THETA[s, ] <- theta_j

# ---- Step 4: Metropolis–Hastings update for global theta (Beta-centered proposal) ----
kappa <- 20  # tuned for ~25–40% acceptance
eps   <- 1e-8

# Safeguard: keep current state strictly inside (0,1)
global_theta <- min(max(global_theta, eps), 1 - eps)

# Proposal parameters (state-dependent, asymmetric)
a_prop <- global_theta * kappa
b_prop <- (1 - global_theta) * kappa

# Propose new global theta
proposed_global_theta <- rbeta(1, a_prop, b_prop)
proposed_global_theta <- min(max(proposed_global_theta, eps), 1 - eps)

# Safeguard: ensure theta_j values are bounded away from 0 and 1
theta_j <- pmin(pmax(theta_j, eps), 1 - eps)  # vector of local inclusion probs

# ----- Log target: numerator vs denominator -----
# Target uses product_j Beta(theta_j ; theta, 1 - theta) * Beta(theta ; alpha1, alpha2)
# dbeta() includes the Beta normalizing constant, so this is correct.

# Numerator: at proposed_global_theta
log_num <- sum(dbeta(theta_j, proposed_global_theta, 1 - proposed_global_theta, log = TRUE)) +
           dbeta(proposed_global_theta, alpha1, alpha2, log = TRUE)

# Denominator: at current global_theta
log_den <- sum(dbeta(theta_j, global_theta, 1 - global_theta, log = TRUE)) +
           dbeta(global_theta, alpha1, alpha2, log = TRUE)

# ----- Hastings correction (proposal density ratio) -----
# q(theta* | theta^(s)) = Beta(kappa*theta^(s), kappa*(1 - theta^(s)))
# q(theta^(s) | theta*) = Beta(kappa*theta*,   kappa*(1 - theta*))
log_q_fwd <- dbeta(proposed_global_theta, a_prop, b_prop, log = TRUE)
log_q_rev <- dbeta(global_theta, proposed_global_theta * kappa, (1 - proposed_global_theta) * kappa, log = TRUE)
...
```{r}
# Save the results
save(BETA, ZLV,THETA,GLOBAL_THETA,theta_j, file = "CAD-Proposed model.RData")

# To load the results back in future sessions
load("CAD-Proposed model.RData")
...
# Run diagnostics
plot_trace_density_to_pdf(BETA_thinned, pretty_labels, "TraceDensity_Beta_PostThinning_flexible.pdf")
plot_trace_density_to_pdf(BETA_ZLV_thinned, pretty_labels, "TraceDensity_BetaZ_PostThinning_flexible.pdf")
plot_acf_to_pdf(BETA_thinned, pretty_labels, "ACF_Beta_PostThinning_flexible.pdf")
plot_acf_to_pdf(BETA_ZLV_thinned, pretty_labels, "ACF_BetaZ_PostThinning_flexible.pdf")
...
pdf("TraceDensity_GlobalTheta_PostThinning_flexible.pdf", width = 8, height = 4)
par(mfrow = c(1, 2), mar = c(4, 4, 3, 1))
...
# ACF plot
pdf("ACF_GlobalTheta_PostThinning_flexible.pdf", width = 6, height = 4)
...
pdf("CI_PSP_Paper_flexible.pdf", width = 12, height = 7)
...
pdf("CI_PSP_Presentation_flexible.pdf", width = 12, height = 7)
...
write.csv(csv_out, "OR_PSP_AllPredictors_flexible.csv", row.names = FALSE)
...
writeLines(latex_code_paper, con = "OR_PSP_Table_NoIntercept_PAPER_flexible.tex")
...
writeLines(latex_code_present, con = "OR_PSP_Table_NoIntercept_PRESENT_flexible.tex")
